...
#set -x
if [ $# -ne 8 ]; then
	echo "[$0] [ERROR] [`date "+%Y-%m-%d %H:%M:%S"`] [cube_query_click] input error ! \$#=$# not 8"
	exit -1
fi
hadoop=$1;hive=$2;input_ready=$3;jobname=$4;main_dir=$5;date=$6;task_prefix=$7;now=$8

cur_dir="${main_dir}/cube_jobs/measure/query_click"
...
sql_file="${cur_dir}/sql/stat_query_click.sql"
...
#-------load query_data------------
python ${cur_dir}/../../../tool/load2MySQL.py -n 12 -t query_data -f $data_file
check "load query data"
#python ${cur_dir}/load_query_data.py $data_file
