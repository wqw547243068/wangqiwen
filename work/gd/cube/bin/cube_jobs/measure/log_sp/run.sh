...
#set -x
if [ $# -ne 8 ]; then
	echo "[$0] [ERROR] [`date "+%Y-%m-%d %H:%M:%S"`] [cube_log_sp] input error ! \$#=$# not 8"
	exit -1
fi
hadoop=$1;hive=$2;input_ready=$3;jobname=$4;main_dir=$5;date=$6;task_prefix=$7;now=$8
cur_dir="${main_dir}/cube_jobs/measure/log_sp"
#note
:<<note
cur_dir='.'
...
sql_file="${cur_dir}/sql/stat_log_sp.sql"
...
python ${cur_dir}/../../../tool/load2MySQL.py -n 19 -t $mysql_table -f $data_file  
