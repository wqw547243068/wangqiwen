...
def getFileSize(job_spec_conf,hadoop,path,date):
    # 2014-7-7 aos&sug path difference from sp&client
    #if job_spec_conf.job_name == 'log_aos':
    if job_spec_conf.job_name == 'log_aos' and 'aos_dxp' in path:
        # /user/ops/flume/aos_new/aos_dxp/2014/06/18/*/*
        # /user/ops/flume/aos/aos_dxp/2014/08/11/*/* 2014-8-12
        path += '/*/*'
    elif job_spec_conf.job_name == 'log_sug':
        # /user/ops/flume/sug/old/20140706/2014/07/06
        #path += '/'+date[0:4]+'/'+date[4:6]+'/'+date[6:8]
        pass # 2014-7-10 back to flume sug
    ...
    if code != 0:
        ...
        if global_conf.diff_on == 'on':
            title = '[session][ERROR] [%s] : fail to get size of hadoop file (%s)  !'%(job_spec_conf.job_name)
...
def runOneTask(job_spec_conf, date, options):
    ...
    while True:
        ...
        if options.enable_timeout and now_time >= end_time:
            if match:
                ...
                title = 'session-%s: Data missed (%s)'%(job_spec_conf.job_name,job_spec_conf.input_ready)
    ...
    for i in all_list:
        if i[-2] == 'NO':
            ...
            break
    if size_diff:
        title = '[session][%s][%s] source file size diff too much !'%(job_spec_conf.job_name,date)
    ...
    if code != 0:
        ...
        title = '[session][ERROR] [%s] Job failed when running !'%(job_spec_conf.job_name)
    ...
    if code != 0:
        ...
        title = '[session][ERROR][%s] Failed to create ready file !'%(job_spec_conf.job_name)
    ...
    if result_dict['miss_num'] > 0:
        ...
        title = '[session][WARNING][%s] Job finished at the end without sources(%s)' %(job_spec_conf.job_name,miss_source_info)
