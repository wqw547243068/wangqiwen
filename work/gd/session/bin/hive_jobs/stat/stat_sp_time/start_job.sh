...
#===============================================================================
#            File:  start_job.sh [stat_sp_time]
#           Usage:  
#     Description:  stat_sp_time表启动程序
...
set -x
if [ $# -ne 8 ]; then
	echo "[$0] [ERROR] [`date "+%Y-%m-%d %H:%M:%S"`] [stat_sp_time] input error ! \$#=$# not 8"
	exit -1
fi
...
${hive} -e "select * from log_data.stat_sp_time where dt='$date'" > $data_file
if [ $? -ne 0 ];then
