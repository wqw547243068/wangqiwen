...
#8个参数 /home/devuse/bin/hadoop/bin/hadoop /home/devuse/bin/hadoop/bin/hive /user/hive/warehouse/logamap.db/log_rc/dt=20140301/000000_0 log_aos ./ 20140301 start_20140301_log_aos 20140616
#:<<note
set -x
if [ $# -ne 8 ]; then
...
#sql="$path/create_aos.sql"
sql="$path/create_aos.sql" # 2014-7-6
date1="${date:0:4}/${date:4:2}/${date:6:2}"
# 启动hive任务
${hive} -hivevar path=$path  -hivevar date1=$date1 -hivevar date=$date -f $sql
if [ $? -ne 0 ];then
