...
create external table if not exists log_aos_old_raw
    (
        line string comment 'raw data from HDFS'
    )
    partitioned by (dt string)
    location "/user/ops/flume/aos/";
...
alter table log_aos_old_raw drop if exists partition (dt='${hivevar:date}');
alter table log_aos_old_raw add partition(dt='${hivevar:date}') location '${hivevar:date1}';
...
select transform(*) 
    ...
    from 
    (   
        select * from log_aos_old_raw where dt='${hivevar:date}' union all 
