...
if __name__ == '__main__':
    ...
    resp_list_2_0_father = ['pos','name','district','adcode','category','rank','poiid','address','x','y','type','distance','display_info','column','null'] 
    ...
    pattern_gb = re.compile(r'(gb|GB)',re.I)
    for line in sys.stdin:
        ...
        try:
            ...
            print >>sys.stderr,"coding transform error ! (%s => %s) continue" %(coding_name,'utf-8')
            continue
        # chardet 
        try:
            coding_result = chardet.detect(line)
                ...
                if in_dict['type'] == 'hive':
                    ...
                    if response_len < 2: # sug无结果
                        ...
                        if len_tmp_f == 1:
                            ...
                            s1 = tmp_f_list[1]
                            try:
                                coding_result = chardet.detect(s1)
                                coding_name = coding_result['encoding'] 
                                if pattern_gb.match(coding_name):
                                    s2 = s1.decode(coding_name).encode('utf-8')
                                else:
                                    s2 = s1
                            except Exception,err:
                                print >>sys.stderr,"coding transform error ! (%s => %s) (%s)" %(coding_name,'utf-8',err)
                            tmp_f_list[1] = s2
